...
<fmt:setLocale value="${param.lang != null ? param.lang : 'vi'}" />
                ...
                <div class="list-group list-group-flush">
                    <c:forEach var="citem" items="${categories}">
                        <a class="list-group-item ${citem.id == activeCatId ? 'active' : ''}"
                           href="${pageContext.request.contextPath}/home?cat=${citem.id}&lang=${empty lang ? 'vi' : lang}">
            ...
            <form action="${pageContext.request.contextPath}/home" method="get" class="mb-3">
                <div class="input-group">
                    <input type="text" name="keyword"
                           class="form-control"
                           placeholder="<fmt:message key='product.search'/>"
                           value="${param.keyword}">
                    <button class="btn btn-primary" type="submit">
                        <i class="fa fa-search"></i> <fmt:message key="btn.search"/>
                    </button>
                </div>
            </form>
                                ...
                                <div class="card h-100">
                                    ...
                                    <div class="card-footer bg-white">
                                        <a class="btn btn-sm btn-primary w-100"
                                           href="${pageContext.request.contextPath}/products/detail?id=${p.id}&lang=${empty lang ? 'vi' : lang}">
